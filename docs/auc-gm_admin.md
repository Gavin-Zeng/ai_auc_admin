...
本文面向后续要用 Codex AI 新建“客服系统”后台的工程师。目标是：新后台基于 Laravel 13 + Inertia Vue 开发，通过现有 AUC 完成统一登录、统一入口、统一权限快照同步。

这份文档是专项接入文档。通用 Laravel 子系统接入说明仍保留在 `docs/auc-laravel-subsystem-integration.md`，本文在它的基础上，改成更贴近“客服系统”场景的落地版本。
...
- 用户先登录 AUC。
- 用户在 AUC 工作台点击“客服系统”系统卡片。
- 浏览器跳转到 `gm.bingniao.ai`。
...
127.0.0.1 gm.bingniao.ai
```

说明：

- `localauc.bingniao.ai`：本地 AUC 域名。
- `gm.bingniao.ai`：客服系统后台本地域名。

如果 AUC 和子系统都跑在本机，Laravel 启动后应保证这两个域名都能访问到对应服务。

## 3. AUC 端系统配置

在 AUC 后台“系统管理”中创建“客服系统”系统，建议按下面配置：

| 字段 | 建议值 | 说明 |
| --- | --- | --- |
| 名称 | 客服系统 | AUC 工作台展示名 |
| 客户端 ID | `gm` | 子系统唯一标识，也是 SSO `client_id` |
| 客户端密钥 | 创建时生成 | 只展示一次，后续只能轮换 |
| 基础地址 | `http://gm.bingniao.ai` | 子系统首页地址 |
| 回调地址 | `http://gm.bingniao.ai/sso/callback` | 必须与子系统完全一致 |
...
```json
{
  ...
  "code": "one-time-code",
  "redirect_uri": "http://gm.bingniao.ai/sso/callback"
}
```
...
客服系统的接口鉴权必须在服务端执行，不能依赖 AUC 菜单隐藏：
...
1. 用户已登录 AUC，点击“客服系统”后，跳到 `gm.bingniao.ai`
...
- 先看 callback 响应和后端日志
- 确认子系统 `.env` 中 `APP_URL`、`SESSION_DOMAIN` 已设置为 `gm.bingniao.ai` 体系
- 确认浏览器 Cookie 已正确写入
